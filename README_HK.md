<div align="center">

# 🚀 CSJSS APPS Hub
## The Ultimate App Store for CSJSS Students & Staff
<br><br><br>
<img src="https://github.com/AriesLinux/CSJSS-Healthy-Kit/blob/main/school_logo.jpg" width="450" alt="School Logo">
<br><br><br>
<br>
<img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_APPS.png" width="120" alt="CSJSS_APPS"><img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_ONLINE.png" width="120" alt="CSJSS_ONLINE"><img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_Leaders.png" width="120" alt="CSJSS_Leaders"><img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_CampusNews.png" width="120" alt="CSJSS_CampusNews"><img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_Go!.png" width="120" alt="CSJSS_Go!"><img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_PASS.png" width="120" alt="CSJSS_PASS"><img src="https://github.com/AriesLinux/CSJSS-Healthy-Kit/blob/main/CSJSS%20Healthy%20Kit.png" width="120" alt="CSJSS_Lab"><img src="https://github.com/AriesLinux/CSJSS-APPS-Hub/blob/main/CSJSS_Lab.png" width="120" alt="CSJSS_Lab">
<br><br>

![Platform](https://img.shields.io/badge/Platform-iOS_%7C_Android-gray?style=for-the-badge&logo=apple&logoColor=white)
![Ecosystem](https://img.shields.io/badge/Ecosystem-CSJ_Integrated-blue?style=for-the-badge&logo=google-cloud&logoColor=white)
![Auth](https://img.shields.io/badge/Auth-CSJSS_SSO-green?style=for-the-badge)
![School](https://img.shields.io/badge/School-Caritas_St._Joseph-red?style=for-the-badge)

<p align="center">
  <strong>明愛聖若瑟中學專屬 App 發布平台 & 數碼校園生態圈</strong>
</p>

[English](./README_EN.md) | [繁體中文](./README.md) | [粵語版本](./README_HK.md)

</div>

---

## 📱 應用矩陣 (App Matrix)

**CSJSS APPS** 唔單止係一個 Download 器，佢係整合全校數碼資源嘅指揮中心，下面係目前有得用嘅 Core Apps：

| 應用名稱 (App Name) | 類別 (Category) | 主要功能 (Key Features) |
| :--- | :--- | :--- |
| **🌐 CSJSS ONLINE** | [Web Portal] | 學校官網變身做 App，支援重要通告 Push Notification（推播通知） |
| **👔 CSJSS Leaders** | [Management] | **領袖生/老師專用**，用嚟出當值時間表，睇住學生秩序紀錄 |
| **📰 CSJSS CampusNews**| [Media] | **學生自己整嘅新聞台**，可以寫文貼圖，Upload 片同埋即時留言吹水 |
| **🏃 CSJSS Go!** | [Activity] | 報名校內活動，查 House (社) 分，睇 ECA 資訊 |
| **💳 CSJSS PASS** | [Digital ID] | **Flagship 功能**，整合 Apple Pay / Google Pay 嘅電子學生證 |
| **🧪 CSJSS Lab** | [Education] | 實驗室安全守則，**AR/VR 科學實驗模擬 Game** |

---

## 📊 平台運行狀態 (Store Status)

### ✅ 核心服務 (Core Services)
* [x] **統一身份認證 (SSO)**: 一鍵 Login 晒所有 CSJSS 系列 App，唔使由頭入過 Account
* [x] **OTA 自動更新**: 當子 App 有新 Version，個 Store 會自動彈 Update 通知
* [x] **權限分級**: 自動認你是邊個（Login CSJSS Google account）（學生/領袖生/老師），開返對應嘅 App 下載權限
* [x] **跨平台支援**: 完美支援 iOS (IPA) 同 Android (APK) 環境（註：CSJSS APPS 喺 iOS 唔需要裝 App，Store 頁面會變做一個 App 整合包）
* [x] **Beta 測試版軟件分發**：可以搶先試玩仲開發緊嘅 Beta 版 App
* [x] **CSJSS Healthy Kit**：基於 EazyUI 自研架構同 MIT App Inventor 開發嘅智能健康監測 App，靠 iRED 公司整嘅 Smart Learning HEALTH KIT 硬件

### ❌ 已知問題 (Known Issues)
* [ ] [CSJSS Lab] 暫時仲係概念階段 (Concept Only)
* [ ] [CSJSS CampusNews] 暫時仲係概念階段 (Concept Only)
* [ ] [CSJSS ONLINE] 暫時仲係概念階段 (Concept Only)
* [ ] [CSJSS Leaders] 暫時仲係概念階段 (Concept Only)
* [ ] [CSJSS Go!] 暫時仲係概念階段 (Concept Only)
* [ ] [CSJSS PASS] 開發慢咗同埋有阻滯

---

## 💳 CSJSS PASS 核心亮點 (Spotlight)

**將你張學生證放入手機 Wallet，體驗「直行直過」嘅方便**

* **Wallet Integration (原生銀包整合)**:
    * 支援一撳加入 **Apple Wallet** (iPhone/Apple Watch)
    * 支援一撳加入 **Google Wallet** (Android)
* **Tap to Check-in (NFC 拍卡)**:
    * 朝早返學嗰陣，唔使解鎖手機或者開 App，直接 **NFC 拍卡** 就記到返學時間
    * 圖書館借書、Lab 簽到、小食部驗證身份，一拍就得
* **Security (安全驗證)**:
    * 動態加密 QR Code，杜絕 Cap 圖借證同盜用身份
      <br><br><br><br>
* **想知多啲去 CSJSS-PASS 嗰邊睇**:

---

## 🧪 CSJSS Lab 創新體驗 (Innovation)

**打破物理限制，將 Science Lab 放入褲袋**

1.  **安全守則資料庫**: 做實驗前必讀 Guide，內置測驗，Pass 咗先准入實體 Lab
2.  **AR 元素週期表**: 用手機鏡頭 Scan 書入面啲圖，3D 浮個化學元素出嚟睇
3.  **VR 虛擬實驗**: 模擬危險化學反應（好似鈉放入水、稀釋強酸），等同學喺安全環境下睇大爆炸

---

## ⚙️ 安裝與部署 (Deployment)

**因為呢個係校內專用 Store，麻煩跟住下面步驟裝：**

1.  **下載安裝包**:
    * 去校網 Download `CSJSS_Store_Installer` (iOS 描述檔 或 Android APK)
2.  **信任企業憑證 (iOS)**:
    * 設定 -> 一般 -> VPN 與裝置管理 -> 㩒信任 `Caritas St. Joseph Secondary School`
3.  **身份登入**:
    * 用學校 Google Account (`@csjss.edu.hk`) Login 個 Store，系統會自動派對應權限畀你
4.  **一鍵部署**:
    * 㩒首頁個 "Install All Essential Apps" 就可以一次過 Download 晒必備工具

---

## 👏 開發貢獻者 (Credits)

* **Project Lead**: CSJ IT 部
* **CSJSS Lab**: Science Society & STEM 部 (提供科學內容同 VR Modeling)
* **UI/UX Design**: EazyUI
